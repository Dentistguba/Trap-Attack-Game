package
{
	...
	public class staticCollisionOct extends collisionObject
	{
		private var world;
		...
		public function staticCollisionOct(World,X,Y,Width,Height,Optional = false):void
		{
			optional = Optional
			world = World;
			x = X;
			...
			width = Width;
			height = Height;
			normalList = new Array([1,0],[0,1],[1,1],[1,-1]);
			pointList = new Vector.<Point>();
			pointList.push(new Point(width/3,0),new Point(0,height/3),new Point(0,(height/3) * 2),new Point(width/3,height),new Point((width/3) * 2,height),new Point((width,height/3) * 2),new Point(width,height/3),new Point((width/3) * 2,0));
		}
