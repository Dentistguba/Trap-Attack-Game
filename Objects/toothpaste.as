package
{
	...
	public class toothpaste extends slippingTrap
	{
		public function toothpaste(level:Level,X = 0,Y = 0,speed = 0,bag:Class = null)
		{
			super(level,X,Y,new toothpasteGraphic());
			friction = 0.02;
		...
		override protected function trapCollide():void
		{
			...
			{				
				if (neighbors[i] is Trap && neighbors[i] != null && neighbors[i] != this && !(noCollideObject != null && neighbors[i] is noCollideObject) && neighbors[i].hitBox != null)
				{
					...
					{
						if (neighbors[i] is toothpaste || neighbors[i] is bleach || neighbors[i] is oil || neighbors[i] is water)
						{
