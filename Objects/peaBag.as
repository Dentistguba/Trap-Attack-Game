package
{
	...
	public class peaBag extends trippingTrap
	{
		private var peaCount:int = 8;
		private var peaDirection:int = 0;
		private var peaTimer:int = 0;
		
		public function peaBag(level:Level,X = 0,Y = 0)
		{
			super(level,X,Y,new peaBagGraphic());
			noCollideObject = pea;
		}
		...
		override protected function mouseInteract(evt:MouseEvent):void
		{
			...
			addEventListener(Event.ENTER_FRAME, addpea);
		...
		private function addpea(evt:Event)
		{
			if (peaCount > 0)
			{
				if (peaTimer == 0)
				{
					if (peaDirection == 0)
					{
						level.trapList.push(level.addChild(new pea(level,x + (width/2),y,5)));
					}
				...
				removeEventListener(Event.ENTER_FRAME, addpea);
		}
	}
}