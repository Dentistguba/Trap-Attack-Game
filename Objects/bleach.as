package
{
	...
	public class bleach extends slippingTrap
	{
		public function bleach(level:Level,X = 0,Y = 0,speed = 0,bag:Class = null)
		{
			super(level,X,Y,new bleachGraphic());
			friction = 0.02;
		...
		override protected function trapCollide():void
		{
			...
			{				
				if (neighbors[i] is Trap && neighbors[i] != null && neighbors[i] != this && !(noCollideObject != null && neighbors[i] is noCollideObject) && neighbors[i].hitBox != null)
				{
					...
					{
						if (neighbors[i] is bleach || neighbors[i] is oil)
						{
