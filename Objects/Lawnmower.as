package
{
	...
	public class Lawnmower extends trippingTrap
	{
		private var accelerated:Boolean = false;
		
		public function Lawnmower(level:Level,X = 0,Y = 0)
		{
			super(level,X,Y,new LawnmowerGraphic());
			friction = 0;
			loopSound = new lawnmowerSound();
		}
		...
		override public function loop():void
		{
			...
			if (accelerated == false && horizontalSpeed > -5)
				horizontalSpeed -= 0.5;
				
			else if (accelerated == true && horizontalSpeed != -5)
			{
				soundChannel.stop();
				looping = false
			}
			
			else if (accelerated == false)
				accelerated = true;
