package
{
	...
	public class marbleBag extends trippingTrap
	{
		private var marbleCount:int = 5;
		private var marbleDirection:int = 0;
		private var marbleTimer:int = 0;
		
		public function marbleBag(level:Level,X = 0,Y = 0)
		{
			super(level,X,Y,new marbleBagGraphic());
			noCollideObject = marble;
		}
		...
		override protected function mouseInteract(evt:MouseEvent):void
		{
			...
			addEventListener(Event.ENTER_FRAME, addMarble);
		...
		private function addMarble(evt:Event)
		{
			if (marbleCount > 0)
			{
				if (marbleTimer == 0)
				{
					if (marbleDirection == 0)
					{
						level.trapList.push(level.addChild(new marble(level,x + (width/2),y,5)));
					}
				...
				removeEventListener(Event.ENTER_FRAME, addMarble);
		}
	}
}